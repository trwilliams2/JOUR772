...
Running the problems function produces a table of all the parsing problems in 350 rows.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: vendor: text; amount: number; date: text**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer: 549; perhaps a missing zero?**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: Some data does not have a category (NA).**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer:** Inconsistent capitalization in names
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer:** Makes the column names lowercase and with underscores instead of spaces.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer:** 50 rows, so 49 possible duplicates, but there are several names with duplicates, so it's less than that.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer:** The new zip5 column has the 5-digit codes.
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? Answer: 21202. That makes sense, as it's a Baltimore zip, dense with people and tourism dollars.
