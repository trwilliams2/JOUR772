...
**Answer: Storing in a new variable so we can use that variable for functions later.**
...
#the first name after mutate is the new column (replacing the old one in this instance)
head(maryland_expenses)
```

**Answer: We ran a function to change the character format in expenditure date to date format and replace the column with a column of the same name and new data.**
...
**Answer: It's changing the dates into the first of the month of each date.**
...
1.  Courses: What are the departments with the least number of courses? Any surprises?

2.  How many advanced level courses (400 and above) are there in the journalism department?

3.  How many data journalism courses (that we can tell from generic title searches like "data") are there in the journalism department?
