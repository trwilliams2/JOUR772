...
md_expenses |> mutate (expenditure_date = mdy(expenditure_date))
```
...
```{r}
# One of them is "Media". Write code to isolate Media expenses where the name of the payee is not NA. The key here is using (!is.na(payee_name)) within a function.
...
filter(payee_clean=="Facebook") |> group_by(committee_name) |> summarise(total_amount = sum(amount)) |> arrange(desc(total_amount))
```
...
**A2.** Jain Ashwani was a candidate for Maryland governor. Peter Franchot and his running mate Monique Anderson-Walker were also candidates in that election, as were Jon Baron and John King.. Billy Boniface was a candidate for Harford County executive. Amanda Fielder was a local council candidate. The Working Families Party supports progressive candidates. The Firefighters of Frederick County is a union group.\
\
Most or all of the 2022 Facebook ads for the Ashwani group are marked: "This content was removed because the disclaimer didn’t follow our [policy for ads about social issues, elections or politics](https://l.facebook.com/l.php?u=https%3A%2F%2Ftransparency.fb.com%2Fpolicies%2Fad-standards%2FSIEP-advertising%2FSIEP&h=AT1beqdyKFXLHwDV5F0zGbGgNmGbmkNLxlNgvGJv7IoINlGjmtZkTsJVjRVxyhz86BiGy4MIQi29W4bXkwt8Od67nJ2-dCe39txnhVNdmoRm03EGB8SXU118qwCpGAMfQ-9pjjRaP5cAaVrl6ebzremOOi8)."
...
**A3.** The top primary vote-getting Republicans and Democrats are missing. SKDK and a distant second, Foglamp (I hope, unless I missed a committee name!).
...
**A4.** It's a totally different list basically. I don't think so, but not sure how to check.

```{r}
media_expenses_clean <- read_csv("data/media-expenses-clean.csv") |>
filter(expense_purpose=="Online Advertising") |> group_by(committee_name) |> summarise(total_amount = sum(amount)) |> arrange(desc(total_amount))
```
...
**A5.** I assume that not all the spending on Facebook has been properly labeled, but that could be because of a variety of maybe interesting or maybe boring reasons. The most interesting thing so far I think is the top Facebook ad spender had his ads removed. Would be interesting to try find out why.
