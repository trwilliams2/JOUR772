...
overdose_calls <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> 
  clean_names() |> 
  rename(date = x1, time = x2, case_number = x3, type = x4, address = x5) |> 
...
A1. There appears to be a tie: July 14th and October 4. There are only 326 rows. The data starts from February 6, which explains the missing days/rows.
...
A2. There appear to be somewhat fewer calls on Thursdays, and more on weekends (Fri-Sun), which probably matches with increased free time for drug use on weekends. Not sure how to do the percentage.
...
A3. The most frequent caller appears to be an ordinary home. The next most appear to be police precincts, which may or may not reflect the actual address. And not sure!
...
A4. Well, there are certainly privacy concerns in having addresses visible in this way, as a location with frequent calls could be a target for harassment due to drug stigma. Not sure if that's a story without more info on how the data is collected/labeled, however.
